...
// Product API Routes Removed (Refactored to use Postgres directly in simpleHandler.js)

let currentClient = null;

// Global status tracking
let lastConnectionStatus = 'offline';

io.on('connection', (socket) => {
    console.log('Web Interface Connected');

    // Send current status immediately on connection
    socket.emit('connection-status', lastConnectionStatus);

    socket.on('pair-device', async (phoneNumber) => {
        if (!currentClient) {
            socket.emit('pairing-error', 'Client not ready. Please wait.');
            return;
        }
        try {
            if (!currentClient.authState.creds.registered) {
                const code = await currentClient.requestPairingCode(phoneNumber, config().setPair);
                socket.emit('pairing-code', code);
            } else {
                socket.emit('pairing-error', 'Client already registered.');
            }
        } catch (err) {
            socket.emit('pairing-error', err.message || 'Pairing failed');
        }
    });
});
...
global.webServer = server.listen(3000, () => {
    console.log('Web Configuration Server running on port 3000');
});
const { Boom } = require('@hapi/boom');
const { color } = require('./System/lib/color');
const { smsg, sleep } = require('./System/lib/myfunction');
...
const clientstart = async () => {
    ...
    if (config().status.terminal && !client.authState.creds.registered) {
        console.log('Waiting for pairing via Web Interface at http://localhost:3000');
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            const textMessage = mek.message.conversation || mek.message.extendedTextMessage?.text || Object.keys(mek.message)[0];
            ...
            require("./simpleHandler")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    client.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') lastConnectionStatus = 'online';
        if (connection === 'close') lastConnectionStatus = 'offline';
        if (connection === 'connecting') lastConnectionStatus = 'connecting';

        const { konek } = require('./System/lib/connection/connect')
        konek({ client, update, clientstart, DisconnectReason, Boom, io })
    })
...
// File watcher disabled to prevent auto-restart
